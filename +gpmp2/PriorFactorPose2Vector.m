...
classdef PriorFactorPose2Vector < gtsam.NoiseModelFactor
  ...
  methods
    function obj = PriorFactorPose2Vector(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gpmp2_wrapper(406, varargin{2});
        end
        base_ptr = gpmp2_wrapper(405, my_ptr);
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'gpmp2.Pose2Vector') && isa(varargin{3},'gtsam.noiseModel.Base')
        [ my_ptr, base_ptr ] = gpmp2_wrapper(407, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gpmp2_wrapper(408, obj.ptr_gpmp2PriorFactorPose2Vector);
    end
