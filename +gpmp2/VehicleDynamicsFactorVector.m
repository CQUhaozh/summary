...
classdef VehicleDynamicsFactorVector < gtsam.NoiseModelFactor
  ...
  methods
    function obj = VehicleDynamicsFactorVector(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gpmp2_wrapper(201, varargin{2});
        end
        base_ptr = gpmp2_wrapper(200, my_ptr);
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'numeric') && isa(varargin{3},'double')
        [ my_ptr, base_ptr ] = gpmp2_wrapper(202, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gpmp2_wrapper(203, obj.ptr_gpmp2VehicleDynamicsFactorVector);
    end
