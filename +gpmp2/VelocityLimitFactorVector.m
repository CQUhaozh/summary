...
classdef VelocityLimitFactorVector < gtsam.NoiseModelFactor
  ...
  methods
    function obj = VelocityLimitFactorVector(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gpmp2_wrapper(177, varargin{2});
        end
        base_ptr = gpmp2_wrapper(176, my_ptr);
      elseif nargin == 4 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.noiseModel.Base') && isa(varargin{3},'double') && isa(varargin{4},'double')
        [ my_ptr, base_ptr ] = gpmp2_wrapper(178, varargin{1}, varargin{2}, varargin{3}, varargin{4});
      else
    ...
    function delete(obj)
      gpmp2_wrapper(179, obj.ptr_gpmp2VelocityLimitFactorVector);
    end
