...
classdef VehicleDynamicsFactorPose2 < gtsam.NoiseModelFactor
  ...
  methods
    function obj = VehicleDynamicsFactorPose2(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gpmp2_wrapper(193, varargin{2});
        end
        base_ptr = gpmp2_wrapper(192, my_ptr);
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'numeric') && isa(varargin{3},'double')
        [ my_ptr, base_ptr ] = gpmp2_wrapper(194, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gpmp2_wrapper(195, obj.ptr_gpmp2VehicleDynamicsFactorPose2);
    end
