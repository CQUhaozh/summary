...
classdef VehicleDynamicsFactorPose2Vector < gtsam.NoiseModelFactor
  ...
  methods
    function obj = VehicleDynamicsFactorPose2Vector(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gpmp2_wrapper(197, varargin{2});
        end
        base_ptr = gpmp2_wrapper(196, my_ptr);
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'numeric') && isa(varargin{3},'double')
        [ my_ptr, base_ptr ] = gpmp2_wrapper(198, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gpmp2_wrapper(199, obj.ptr_gpmp2VehicleDynamicsFactorPose2Vector);
    end
