...
classdef GoalFactorArm < gtsam.NoiseModelFactor
  ...
  methods
    function obj = GoalFactorArm(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gpmp2_wrapper(169, varargin{2});
        end
        base_ptr = gpmp2_wrapper(168, my_ptr);
      elseif nargin == 4 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.noiseModel.Base') && isa(varargin{3},'gpmp2.Arm') && isa(varargin{4},'gtsam.Point3')
        [ my_ptr, base_ptr ] = gpmp2_wrapper(170, varargin{1}, varargin{2}, varargin{3}, varargin{4});
      else
    ...
    function delete(obj)
      gpmp2_wrapper(171, obj.ptr_gpmp2GoalFactorArm);
    end
