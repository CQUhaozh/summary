...
classdef NonlinearEqualityCalibratedCamera < gtsam.NoiseModelFactor
  ...
  methods
    function obj = NonlinearEqualityCalibratedCamera(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(2081, varargin{2});
        end
        base_ptr = gtsam_wrapper(2080, my_ptr);
      elseif nargin == 2 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.CalibratedCamera')
        [ my_ptr, base_ptr ] = gtsam_wrapper(2082, varargin{1}, varargin{2});
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.CalibratedCamera') && isa(varargin{3},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(2083, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gtsam_wrapper(2084, obj.ptr_gtsamNonlinearEqualityCalibratedCamera);
    end
    ...
    function varargout = print(this, varargin)
      ...
      if length(varargin) == 1 && isa(varargin{1},'char')
        gtsam_wrapper(2094, this, varargin{:});
      else
    ...
    function varargout = printKeys(this, varargin)
      ...
      % Doxygen can be found at http://research.cc.gatech.edu/borg/sites/edu.borg/html/index.html
      if length(varargin) == 1 && isa(varargin{1},'char')
        gtsam_wrapper(2095, this, varargin{:});
      else
    ...
    function varargout = whitenedError(this, varargin)
      ...
      % Doxygen can be found at http://research.cc.gatech.edu/borg/sites/edu.borg/html/index.html
      if length(varargin) == 1 && isa(varargin{1},'gtsam.Values')
        varargout{1} = gtsam_wrapper(2098, this, varargin{:});
      else
  ...
  methods(Static = true)
    function varargout = string_deserialize(varargin)
      ...
      if length(varargin) == 1
        varargout{1} = gtsam_wrapper(2100, varargin{:});
      else
