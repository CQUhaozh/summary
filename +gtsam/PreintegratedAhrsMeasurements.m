  ...
  methods
    function obj = PreintegratedAhrsMeasurements(varargin)
      if nargin == 2 && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        my_ptr = varargin{2};
        gtsam_wrapper(2309, my_ptr);
      elseif nargin == 2 && isa(varargin{1},'double') && isa(varargin{2},'double')
        my_ptr = gtsam_wrapper(2310, varargin{1}, varargin{2});
      elseif nargin == 1 && isa(varargin{1},'gtsam.PreintegratedAhrsMeasurements')
        my_ptr = gtsam_wrapper(2311, varargin{1});
      else
    ...
    function delete(obj)
      gtsam_wrapper(2312, obj.ptr_gtsamPreintegratedAhrsMeasurements);
    end
