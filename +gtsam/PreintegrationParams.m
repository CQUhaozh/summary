...
classdef PreintegrationParams < gtsam.PreintegratedRotationParams
  ...
  methods
    function obj = PreintegrationParams(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(2230, varargin{2});
        end
        base_ptr = gtsam_wrapper(2229, my_ptr);
      elseif nargin == 1 && isa(varargin{1},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(2231, varargin{1});
      else
    ...
    function delete(obj)
      gtsam_wrapper(2232, obj.ptr_gtsamPreintegrationParams);
    end
    ...
    function varargout = getUse2ndOrderCoriolis(this, varargin)
      ...
      % Doxygen can be found at http://research.cc.gatech.edu/borg/sites/edu.borg/html/index.html
      varargout{1} = gtsam_wrapper(2237, this, varargin{:});
    end
    ...
    function varargout = setOmegaCoriolis(this, varargin)
      ...
      if length(varargin) == 1 && isa(varargin{1},'double') && size(varargin{1},2)==1
        gtsam_wrapper(2243, this, varargin{:});
      else
  ...
  methods(Static = true)
    function varargout = MakeSharedD(varargin)
      ...
      elseif length(varargin) == 0
        varargout{1} = gtsam_wrapper(2246, varargin{:});
      else
    ...
    function varargout = MakeSharedU(varargin)
      ...
      % Doxygen can be found at http://research.cc.gatech.edu/borg/sites/edu.borg/html/index.html
      if length(varargin) == 1 && isa(varargin{1},'double')
        varargout{1} = gtsam_wrapper(2247, varargin{:});
      elseif length(varargin) == 0
        varargout{1} = gtsam_wrapper(2248, varargin{:});
      else
