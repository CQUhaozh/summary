...
classdef Diagonal < gtsam.noiseModel.Gaussian
  ...
  methods
    function obj = Diagonal(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(659, varargin{2});
        end
        base_ptr = gtsam_wrapper(658, my_ptr);
      else
    ...
    function delete(obj)
      gtsam_wrapper(660, obj.ptr_gtsamnoiseModelDiagonal);
    end
  ...
  methods(Static = true)
    ...
    function varargout = Variances(varargin)
      ...
      % Doxygen can be found at http://research.cc.gatech.edu/borg/sites/edu.borg/html/index.html
      if length(varargin) == 1 && isa(varargin{1},'double') && size(varargin{1},2)==1
        varargout{1} = gtsam_wrapper(666, varargin{:});
      else
