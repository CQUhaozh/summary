...
classdef NonlinearEqualityPoint2 < gtsam.NoiseModelFactor
  ...
  methods
    function obj = NonlinearEqualityPoint2(varargin)
      if (nargin == 2 || (nargin == 3 && strcmp(varargin{3}, 'void'))) && isa(varargin{1}, 'uint64') && varargin{1} == uint64(5139824614673773682)
        if nargin == 2
          ...
          my_ptr = gtsam_wrapper(1913, varargin{2});
        end
        base_ptr = gtsam_wrapper(1912, my_ptr);
      elseif nargin == 2 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.Point2')
        [ my_ptr, base_ptr ] = gtsam_wrapper(1914, varargin{1}, varargin{2});
      elseif nargin == 3 && isa(varargin{1},'numeric') && isa(varargin{2},'gtsam.Point2') && isa(varargin{3},'double')
        [ my_ptr, base_ptr ] = gtsam_wrapper(1915, varargin{1}, varargin{2}, varargin{3});
      else
    ...
    function delete(obj)
      gtsam_wrapper(1916, obj.ptr_gtsamNonlinearEqualityPoint2);
    end
    ...
    function varargout = printKeys(this, varargin)
      ...
      % Doxygen can be found at http://research.cc.gatech.edu/borg/sites/edu.borg/html/index.html
      if length(varargin) == 1 && isa(varargin{1},'char')
        gtsam_wrapper(1927, this, varargin{:});
      else
  ...
  methods(Static = true)
    function varargout = string_deserialize(varargin)
      ...
      if length(varargin) == 1
        varargout{1} = gtsam_wrapper(1932, varargin{:});
      else
